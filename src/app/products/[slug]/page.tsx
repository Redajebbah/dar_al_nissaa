...
const accordionItems = [
  ...
  },
  {
    ...
    content: `XS: 34-36 • S: 38-40 • M: 42-44 • L: 46-48 • XL: 50-52 • XXL: 54-56\n\nNos vêtements taillent normalement. En cas de doute, nous recommandons de prendre la taille au-dessus.`,
    key: 'sizes',
  },
  {
    title: 'Livraison & Retours',
    content: `🚚 Livraison gratuite partout au Maroc sous 24-48h\n💳 Paiement cash à la livraison\n🔄 Échange gratuit sous 7 jours si la taille ne convient pas\n📞 Support WhatsApp disponible 7j/7`,
    key: 'delivery',
  },
  {
    title: 'Entretien',
    ...
    <div className="min-h-screen bg-cream pt-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <nav className="flex items-center gap-2 text-sm text-gray-400">
          ...
          <ChevronRight size={14} />
          <span className="text-emerald font-medium line-clamp-1">
            {product.name.fr}
          </span>
        </nav>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16">
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-3">
              <Link
                href={`/collections/${product.category}`}
                className="text-gold text-xs font-semibold tracking-wider uppercase hover:underline"
              >
                {categoryInfo?.nameFr || product.category}
              </Link>
              {product.isNew && (
                <span className="bg-emerald text-white text-xs font-semibold px-2.5 py-1 rounded-full">
                  Nouveau
                </span>
              )}
            ...
            <p className="font-arabic text-xl text-gold mb-4" dir="rtl">
              {product.name.ar}
            </p>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-3">
                <label className="text-sm font-semibold text-emerald uppercase tracking-wider">
                  Couleur
                </label>
                  ...
                  <button
                    ...
                    className={`w-10 h-10 rounded-full transition-all duration-200 border-2 ${
                      selectedColor.name === color.name
                        ? 'border-emerald scale-110 shadow-md'
                        : 'border-transparent hover:scale-105'
                    }`}
                    style={{ background: color.hex }}
                  >
                    {color.hex === '#FAF7F2' || color.hex === '#FFFFF0' ? (
            ...
            {/* Quantity */}
            <div className="mb-8">
              <label className="text-sm font-semibold text-emerald uppercase tracking-wider mb-3 block">
                Quantité
              </label>
              <div className="flex items-center gap-4">
                <div className="flex items-center border-2 border-cream-300 rounded-full overflow-hidden">
                  <button
                    ...
                    className="w-10 h-10 flex items-center justify-center text-emerald hover:bg-cream-200 transition-colors font-bold text-lg"
                  >
                    −
                  </button>
                  <span className="w-12 text-center font-semibold text-emerald">
                    {quantity}
                  </span>
                  <button
            ...
            <div className="grid grid-cols-3 gap-3 mb-8">
              {[
                ...
                <div
                  key={badge.text}
                  className="flex flex-col items-center gap-2 bg-white rounded-xl p-3 text-center shadow-card"
                >
                      ...
                      >
                        <div className="pb-5 text-sm text-gray-600 leading-relaxed whitespace-pre-line">
                          ...
                          {item.key === 'desc' && product.material && (
                            <p className="mt-3 text-emerald font-medium">
        ...
        {related.length > 0 && (
          <section className="mt-20">
            <div className="text-center mb-10">
              <p className="text-gold text-sm font-semibold tracking-[0.3em] uppercase mb-2">
                Vous aimerez aussi
              </p>
              <h2 className="font-serif text-3xl text-emerald">
                Produits Similaires
              </h2>
            </div>
